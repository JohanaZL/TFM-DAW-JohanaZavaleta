...
async function getCategoryProducts(slug: string): Promise<{ products: Product[]; categoryName: string } | null> {
  try {
    const res = await fetch(
      `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL ?? 'http://localhost:3000'}/api/products?category=${slug}`,
      { cache: 'no-store' }
    );
    ...
    const catRes = await fetch(
      `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL ?? 'http://localhost:3000'}/api/categories`,
      { cache: 'no-store' }
    );
